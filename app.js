...
function changeMonth(direction) { currentDate=new Date(currentDate.getFullYear(),currentDate.getMonth()+direction,1);ensureAutomaticIncome();saveState();if(currentView==='cards')renderCards();else renderHome(); }
...
function deleteRecord(id) { const target=state.records.find(item=>String(item.id)===String(id));if(!target)return;const groupedPurchase=target.tipo==='cartao'&&target.parent_id;state.records=state.records.filter(item=>groupedPurchase?item.parent_id!==target.parent_id:String(item.id)!==String(id));saveState();showToast(groupedPurchase?'Compra e todas as parcelas excluídas.':'Registro excluído.');renderAll(); }
